...
class SQLTypesConverterTest {
    ...
    @Test
    fun `sqlite decimal uses getString to avoid precision loss`() {
        val field = Field(1, JDBCType.DECIMAL, "DECIMAL", DbFlavour.SQLITE)
        every { resultSet.getString(1) } returns "123.456"
        val result = sqlTypesConverter.convert(field, resultSet)
        result.shouldBe(BigDecimal("123.456"))
    }

    @Test
    fun `sqlite decimal supports null`() {
        val field = Field(1, JDBCType.DECIMAL, "DECIMAL", DbFlavour.SQLITE)
        every { resultSet.getString(1) } returns null
        val result = sqlTypesConverter.convert(field, resultSet)
        result.shouldBe(null)
    ...
    @Test
    fun `when convertDecimal null, return null`() {
